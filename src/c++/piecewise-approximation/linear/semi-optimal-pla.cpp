...
namespace SemiOptimalPLA {
    ...
    void Compression::__makeSemiConnect(int semi_case) {
        ...
        for (int i=shrink_points.size()-1; i>=0; i--) {
    ...
    void Compression::finalize() {
        ...
        if (this->seg_2->u_line == nullptr) {
            ...
            Point2D last_point = this->seg_1->popLast(false);
            this->curr_end = new Point2D(last_point.x, seg_1->extrm->subs(last_point.x));
            this->yield();
        }
