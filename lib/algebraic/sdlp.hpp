...
{   
    private:
        ...
        bool unit(int d, double *a)
        {
            double mag = 0.0;
            for (int i = 0; i <= d; i++) mag += a[i] * a[i];
            if (mag < (d + 1) * eps * eps) return true;
            
            mag = 1.0 / std::sqrt(mag);
            for (int i = 0; i <= d; i++) a[i] *= mag;
            return false;
        }

        /* optimize the unconstrained objective */
        int lp_no_con(int d, const double *n_vec,
                            const double *d_vec,
                            double *opt)
        {
            double n_dot_d = 0.0;
        ...
        void findimax(int d, const double *pln,
                            int *imax)
        {
            *imax = 0;
        ...
        void vector_up(int d, const double *equation,
                            const int ivar,
        ...
        void vector_down(int d, const double *elim_eqn,
                                const int ivar,
        ...
        void plane_down(int d, const double *elim_eqn,
                            const int ivar,
        ...
        int linfracprog(int d, const double *halves, /* halves  --- half spaces */
                            ...
                            int *prev)            /* prev    --- array of indices into halves */
        {
            ...
            */
            if (d == 1)
            {
                if (m == 0) return lp_no_con(1, n_vec, d_vec, opt);
                else return lp_base_case((const double(*)[2])halves, m, n_vec, d_vec, opt, next, prev);
            }
            ...
            double val = 0.0;
            for (int j = 0; j <= d; j++) val += d_vec[j] * d_vec[j];
            ...
            for (int i = 0; i != m; i = next[i])
            {
                ...
                if (val < -(d + 1) * eps)
                {
                    ...
                    if (d_vec_zero)
                    {
                        ...
                        for (int j = 0; j < d; j++) new_d_vec[j] = 0.0;
                    }
                    else
                    {
                    ...
                    if (status != INFEASIBLE)
                    {
                        ...
                        /* code for unit */
                        double mag = 0.0;
                        for (int j = 0; j <= d; j++) mag += opt[j] * opt[j];
                        mag = 1.0 / sqrt(mag);
                        for (int j = 0; j <= d; j++) opt[j] *= mag;
                    }
                    else
                    {
                        return status;
    ...
    public:
        double warm_linprog(const Eigen::VectorXd &c,
                                const Eigen::MatrixXd &Anew,
                                const Eigen::VectorXd &bnew,
                                Eigen::VectorXd &x)
        {
            int k = bnew.size();       // number of new constraints
            if (k == 0) return c.dot(x);

            int d = x.size();
            int m_old = halves.cols();       // existing constraints
            int m_new = m_old + k;            // total constraints

            // --- Insert new constraints element-wise ---
            halves.conservativeResize(d + 1, m_new);
            work.conservativeResize((m_new + 3) * (d + 2) * (d - 1) / 2);
            for (int j = 0; j < k; ++j)
            {   
                for (int i = 0; i < d; ++i) {
                    halves(i, m_old + j) = -Anew(j, i);
                }
                halves(d, m_old + j) = bnew(j);         // copy b
                // Normalize column
                double norm = halves.col(m_old + j).norm();
                if (norm != 0.0) halves.col(m_old + j) /= norm;
            }
            ...
            prev(0) = perm(perm.size() - 1) + 1;
            /* link the zero position in at the beginning */
            next(0) = perm(0) + 1;
            prev(perm(0) + 1) = 0;
            /* link the other planes */
            for (int i = 0; i < m_new - 2; i++)
            {
                next(perm(i) + 1) = perm(i + 1) + 1;
                prev(perm(i + 1) + 1) = perm(i) + 1;
            }
            /* flag the last plane */
            next(perm(m_new - 2) + 1) = m_new;

            int status = this->linfracprog(d, halves.data(), m_new, m_new, n_vec.data(), d_vec.data(),
                opt.data(), work.data(), next.data(), prev.data());

            // --- Convert homogeneous opt back to x ---
            double minimum = INFINITY;
            if (status != INFEASIBLE)
            {
                if (opt(d) != 0.0 && status != UNBOUNDED)
                {
                    x = opt.head(d) / opt(d);
                    minimum = c.dot(x);
                }
        ...
        */
        {   
            ...
            /* flag the last plane */
            next(perm(m - 2) + 1) = m;
            int status = this->linfracprog(d, halves.data(), m, m, n_vec.data(), d_vec.data(),
                opt.data(), work.data(), next.data(), prev.data());

            /* handle  for linprog whose definitions differ from linfracprog */
            double minimum = INFINITY;
            if (status != this->INFEASIBLE)
            {
                if (opt(d) != 0.0 && status != this->UNBOUNDED)
                {
                    x = opt.head(d) / opt(d);
                    minimum = c.dot(x);
                }
